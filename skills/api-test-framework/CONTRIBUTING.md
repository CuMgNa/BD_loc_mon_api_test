...
本技能位于仓库根目录 `skills/api-test-framework/`（[SKILL.md](SKILL.md) + references + assets/templates），是 jkpt 接口测试用例生成的**唯一真相源**。维护要点：**改代码必改文档，改文档必同步 CHANGELOG**。
...
- 不要在 testcase 直接 `import api_test_framework.*`（该包已删除）
- 不要在 YAML 写可执行表达式
...
本技能是**本仓** jkpt 用例生成的唯一依据。不要按「复制通用层 4 个文件到新项目」来维护；`common/` 新能力必须回写本技能。
